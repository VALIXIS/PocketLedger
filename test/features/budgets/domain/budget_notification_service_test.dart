...
void main() {
  group('BudgetNotificationService Unit Tests', () {
    ...
    test('1. Initial state transition from none to warning emits notification', () {
      final alert = createAlert(budgetId: 'b1', level: BudgetAlertLevel.warning, percentage: 76.0);
    ...
    test('2. Staying in warning (75% to 80%) emits NO new notification (deduplicated)', () {
      final alert1 = createAlert(budgetId: 'b1', level: BudgetAlertLevel.warning, percentage: 76.0);
      final alert2 = createAlert(budgetId: 'b1', level: BudgetAlertLevel.warning, percentage: 80.0);
    ...
    test('3. Escalation from warning to danger emits notification', () {
      final warningAlert = createAlert(budgetId: 'b1', level: BudgetAlertLevel.warning, percentage: 80.0);
      final dangerAlert = createAlert(budgetId: 'b1', level: BudgetAlertLevel.danger, percentage: 92.0);
    ...
    test('4. Escalation from danger to exceeded emits notification', () {
      final dangerAlert = createAlert(budgetId: 'b1', level: BudgetAlertLevel.danger, percentage: 95.0);
      final exceededAlert = createAlert(budgetId: 'b1', level: BudgetAlertLevel.exceeded, percentage: 105.0);
    ...
    test('5. Staying in exceeded (105% to 120%) emits NO new notification', () {
      final exceeded1 = createAlert(budgetId: 'b1', level: BudgetAlertLevel.exceeded, percentage: 105.0);
      final exceeded2 = createAlert(budgetId: 'b1', level: BudgetAlertLevel.exceeded, percentage: 120.0);
    ...
    test('6. De-escalation to none (refund) resets tracking state without emitting notification', () {
      final warningAlert = createAlert(budgetId: 'b1', level: BudgetAlertLevel.warning, percentage: 80.0);
      final noneAlert = createAlert(budgetId: 'b1', level: BudgetAlertLevel.none, percentage: 50.0);
    ...
    test('7. Re-crossing warning threshold after de-escalation emits new notification', () {
      final warning1 = createAlert(budgetId: 'b1', level: BudgetAlertLevel.warning, percentage: 80.0);
      final noneAlert = createAlert(budgetId: 'b1', level: BudgetAlertLevel.none, percentage: 50.0);
      final warning2 = createAlert(budgetId: 'b1', level: BudgetAlertLevel.warning, percentage: 78.0);
    ...
    test('8. Direct transition from none to exceeded (0% to 110%) emits notification', () {
      final exceededAlert = createAlert(budgetId: 'b1', level: BudgetAlertLevel.exceeded, percentage: 110.0);
    ...
    test('9. Independent state tracking across multiple budgets', () {
      final alertB1 = createAlert(budgetId: 'b1', level: BudgetAlertLevel.warning);
      final alertB2 = createAlert(budgetId: 'b2', level: BudgetAlertLevel.danger);

      final emitted = service.processAlerts([alertB1, alertB2]);

      expect(emitted.length, equals(2));
      expect(service.getLastNotifiedLevel('b1'), equals(BudgetAlertLevel.warning));
      expect(service.getLastNotifiedLevel('b2'), equals(BudgetAlertLevel.danger));
    });

    test('10. clearHistory resets notification tracking and log', () {
      final alert = createAlert(budgetId: 'b1', level: BudgetAlertLevel.warning);
